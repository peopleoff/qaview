...
import { emails, images, links, qaChecklist, qaNotes, sendlogAttachments, spellErrors } from "@/lib/db/schema/index";
...
export default defineEventHandler(async (event) => {
  ...
  await db.transaction(async (tx) => {
    ...
    await tx.delete(sendlogAttachments).where(eq(sendlogAttachments.emailId, id));
