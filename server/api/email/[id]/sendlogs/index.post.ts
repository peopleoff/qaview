...
export default defineEventHandler(async (event) => {
  ...
  for (const item of formData) {
    if (item.name === "files" && item.filename && item.data) {
      ...
      const uniqueFilename = `sendlog-${id}-${timestamp}${fileExt}`;

      // Ensure upload directory exists
      const uploadDir = join(process.cwd(), "public/uploads/sendlogs");
      ...
      // Save to database
      const [attachment] = await db.insert(sendlogAttachments).values({
        emailId: id,
        filename: uniqueFilename,
        originalName: item.filename,
        mimeType: item.type || "application/octet-stream",
        size: item.data.length,
        path: `/uploads/sendlogs/${uniqueFilename}`,
        description,
      }).returning();
