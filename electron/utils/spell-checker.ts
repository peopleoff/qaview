...
function getDictionaryDir(): string {
  if (app.isPackaged) {
    // In production, dictionaries are in resources/dictionaries
...
async function initializeDictionary(): Promise<Typo> {
  ...
  try {
    ...
    const dicPath = join(dictDir, "en_US.dic");
    const affPath = join(dictDir, "en_US.aff");
    const customDicPath = join(dictDir, "custom.dic");
